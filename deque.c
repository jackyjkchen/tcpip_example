...
int deque_push_front_(deque_base_t *base, void *node, size_t node_size) {
    ...
    base->nnode++;
#if DEQUE_DEBUG
    fprintf(stderr, "push_front_nodes: %lu\n", base->nnode);
#endif
    return 1;
}
...
int deque_push_back_(deque_base_t *base, void *node, size_t node_size) {
    ...
    base->nnode++;
#if DEQUE_DEBUG
    fprintf(stderr, "push_back_nodes: %lu\n", base->nnode);
#endif
    return 1;
}
...
#if DEQUE_DEBUG
    fprintf(stderr, "base->tailidx: %lu\n", base->tailidx);
    fprintf(stderr, "shrink_size: %lu\n", base->nsize);
#endif
    return base->nsize;
}
